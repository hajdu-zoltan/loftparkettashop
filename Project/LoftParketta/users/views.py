...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Beállítjuk a `backend` attribútumot, hogy ne okozzon hibát a `login` függvény hívásakor
            backend = get_backends()[0]  # Használjuk az első elérhető hitelesítési backend-et
            user.backend = f"{backend.__module__}.{backend.__class__.__name__}"

            # Bejelentkeztetjük a felhasználót a megfelelő backend használatával
            login(request, user)
